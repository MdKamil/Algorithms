...
public class MaximumWidthOfBinaryTree {
	...
	private static class TreeNodeIdentifier {
		TreeNode node;
		long identifier;

		public TreeNodeIdentifier(TreeNode node, long identifier) {
			this.node = node;
	...
	public static int widthOfBinaryTree(TreeNode root) {
		int maximumWidth = 0;
		if (root != null) {
			Queue<TreeNodeIdentifier> queue = new ArrayDeque<>();
			TreeNodeIdentifier currNodeidentifier = new TreeNodeIdentifier(root, 1);
			TreeNodeIdentifier nextLevelFirstNode = currNodeidentifier;
			TreeNodeIdentifier nextLevelLastNode = currNodeidentifier;
			queue.add(currNodeidentifier);
			while (!queue.isEmpty()) {
				TreeNodeIdentifier currNode = queue.poll();
				if (currNode == nextLevelFirstNode) {
					if (nextLevelFirstNode != null && nextLevelLastNode != null) {
						maximumWidth = (int) Math.max(maximumWidth,
								(nextLevelLastNode.identifier - nextLevelFirstNode.identifier) + 1);
					}
					nextLevelFirstNode = null;
					nextLevelLastNode = null;
				}
				TreeNodeIdentifier left = null;
				TreeNodeIdentifier right = null;
				if (currNode.node.left != null) {
					left = new TreeNodeIdentifier(currNode.node.left, currNode.identifier * 2l);
					queue.add(left);
				}
				if (currNode.node.right != null) {
					right = new TreeNodeIdentifier(currNode.node.right, (currNode.identifier * 2l) + 1l);
					queue.add(right);
				}
				if (nextLevelFirstNode == null) {
					nextLevelFirstNode = (left != null ? left : right);
				}
				if (right != null) {
					nextLevelLastNode = right;
				} else if (left != null) {
					nextLevelLastNode = left;
				}
			}
		}
		return maximumWidth;
